...
function mapToFeatured(item = {}) {
  ...
  const dateRaw =
    ...
    "";
  return {
    id: item.id || item.blogId || item.blog_id || "",
    author: item.author || item.created_by || "",
    title: item.title || "Untitled post",
    shortdes:
      item.shortdes ||
      (item.content ? stripHtml(item.content).slice(0, 220) : ""),
    image: normalizeImage(
      item.featured_image_url || item.featuredimage || item.image || PLACEHOLDER
    ),
...
function mapToRecentCard(item = {}) {
  const dateRaw = item.publishdate || item.publishDate || item.created_at || "";
  return {
    id: item.id || item.blogId || item.blog_id || "",
    title: item.title || "Untitled post",
    shortdes:
      item.shortdes ||
      (item.content ? stripHtml(item.content).slice(0, 160) : ""),
...
export default function BlogSectionOne({
  ...
  const categories = Array.isArray(sidebarCategories) ? sidebarCategories : [];
